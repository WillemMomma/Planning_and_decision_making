...
def get_mount_positions(maps, otherrobots, start_position):
    if maps == 0:
        return np.array([(start_position[0], start_position[1], 0)])
    elif maps == 1:
        if otherrobots:
            return np.array([(start_position[0], start_position[1], 0), (6, -8, 0), (-6, 8, 0), (4, 0.5, 0), (-6, -0.5, 0), (-12, -0.5, 0), (12, 0.5, 0)])
        else:
            return np.array([(start_position[0], start_position[1], 0)])
    elif maps == 2:
        if otherrobots:
            return np.array([(start_position[0], start_position[1], 0), (5, -8, 0), (-3, 8, 0), (3, 0.5, 0), (-5, -0.5, 0), (-11, -0.5, 0), (11, 0.5, 0)])
        else:
            return np.array([(start_position[0], start_position[1], 0)])
    elif maps == 3:
        return np.array([(start_position[0], start_position[1], 0), (-7, -4, 0), (-7, -2, 0), (-7, 0, 0), (-7, 2, 0), (-7, 4, 0), (-7, 6, 0), (7, -5, 0), (7, -3, 0), (7, -1, 0), (7, 1, 0), (7, 3, 0), (7, 5, 0), (-7.5, -4, 0), (-7.5, -2, 0), (-7.5, 0, 0), (-7.5, 2, 0), (-7.5, 4, 0), (-7.5, 6, 0), (7.5, -5, 0), (7.5, -3, 0), (7.5, -1, 0), (7.5, 1, 0), (7.5, 3, 0), (7.5, 5, 0)])
    elif maps == 4:
        return np.array([(start_position[0], start_position[1], 0)])

def get_obstacles(maps):
    if maps == 0 or maps == 3:
        return ([(100, 100, 100, 100)])
    elif maps == 1 or maps == 2 or maps == 4:
        from env.gym_envs_urdf.scene_objects.obstacles import walls1, walls2, walls3
        if maps == 1:
            return walls2obstacles(walls1)
        elif maps == 2:
            return walls2obstacles(walls2)
        elif maps == 4:
            return walls2obstacles(walls3)




def initMap(otherrobots, maps, start_position):
    mountPositions = get_mount_positions(maps, otherrobots, start_position)
    obstacles = get_obstacles(maps)
...
def initEnv(mountPositions, trajectory, otherrobots=False, maps=0, dt=0.01):
    ...
    if maps == 3:
        ...
        initialPositions = np.array(
            [
                (0, 0, angle - np.deg2rad(90)), (0, 0, np.deg2rad(270)), (0, 0, np.deg2rad(270)),
                (0, 0, np.deg2rad(270)), (0, 0, np.deg2rad(270)), (0, 0, np.deg2rad(270)), (0, 0, np.deg2rad(270)),
                (0, 0, np.deg2rad(90)), (0, 0, np.deg2rad(90)), (0, 0, np.deg2rad(90)), (0, 0, np.deg2rad(90)),
                (0, 0, np.deg2rad(90)), (0, 0, np.deg2rad(90)),
                (0, 0, np.deg2rad(270)), (0, 0, np.deg2rad(270)), (0, 0, np.deg2rad(270)), (0, 0, np.deg2rad(270)),
                (0, 0, np.deg2rad(270)), (0, 0, np.deg2rad(270)),
                (0, 0, np.deg2rad(90)), (0, 0, np.deg2rad(90)), (0, 0, np.deg2rad(90)), (0, 0, np.deg2rad(90)),
                (0, 0, np.deg2rad(90)), (0, 0, np.deg2rad(90))
        ...
        max_len = max(arr1.shape[0], arr2.shape[0], arr3.shape[0], arr4.shape[0], arr5.shape[0], arr6.shape[0],
                      arr7.shape[0], arr8.shape[0], arr9.shape[0], arr10.shape[0], arr11.shape[0], arr12.shape[0],
                      arr13.shape[0], arr14.shape[0], arr15.shape[0], arr16.shape[0], arr17.shape[0], arr18.shape[0],
                      arr19.shape[0], arr20.shape[0], arr21.shape[0], arr22.shape[0], arr23.shape[0], arr24.shape[0])
        ...
        steeringInput = np.concatenate(
            [arr1, arr2, arr3, arr4, arr5, arr6, arr7, arr8, arr9, arr10, arr11, arr12, arr13, arr14, arr15, arr16,
             arr17, arr18, arr19, arr20, arr21, arr22, arr23, arr24], axis=1)
...
def robotMain(mountPositions, m, pos, vel, current_orientations, omega, otherRobots, env, dt=0.01):
    ...
    if m > 1:
        ...
        for i in range(m - 1):
            ...
            # Create the action array for the other robots
